...
```kotlin
plugins {
    id("de.espirit.firstspirit-module") version "7.0.3"
}
```
...
```kotlin
plugins {
    id("de.espirit.firstspirit-module-annotations") version "7.0.3"
}
```
...
```kotlin
plugins {
    id("de.espirit.firstspirit-module-configurations") version "7.0.3"
}
```
...
| fsWebCompile    | Same as the usual `implementation` configuration, but the dependency and all transitive dependencies are added to the web-resources tag of all web-apps in the module-isolated.xml |
...
* [Gradle](http://www.gradle.org/downloads) 8.11+
