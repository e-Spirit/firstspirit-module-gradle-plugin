 ...
 */
class FsmResources(
    private val fsmResources: Provider<List<ResourceEntry>>,
    ...
    fun fsmResources(): List<Node> {
        ...
        for ((relativePath, node) in fsmResources.get()) {
    ...
    companion object {
        ...
        fun fsmResources(project: Project): List<ResourceEntry> {
            val fsmResources = mutableListOf<ResourceEntry>()
            for ((project, scope) in scopedProjectDependencies(project)) {
                fsmResources.addAll(fsmResources(project, scope))
            }
            return fsmResources
        }

        private fun scopedProjectDependencies(project: Project): List<ScopedProjectDependency> {
    ...
    data class ResourceEntry(val relativePath: String, val node: Node)
