...
- **Test fixture project** at `test-project/` — single Kotlin-DSL composite build with four subprojects (`:module`, `:server`, `:web`, `:fsm`). Consumes the FSMGP under development via `includeBuild("../")`. Built end-to-end on master via `./gradlew buildTestProject` and `./gradlew buildTestProjectWithTemplate` (root-project Exec wrappers that shell into the fixture).
...
These files / external repos sit next to the plugin code and frequently need to be touched alongside it. Reference internal Bitbucket repositories by their `<PROJECT>/<REPO>` pair only — never by URL, since this repo mirrors to public GitHub.

- **`UPGRADE.md`** (repo root) — user-facing upgrade notes for downstream FSM projects. Whenever a change is user-visible (changed defaults, renamed properties, new restrictions like configuration-cache compatibility, dropped features) it must be documented here as part of the same change.
- **`ci.yaml`** (repo root) — Bamboo Plan Generator input. Schema and supported keys are owned by `DEVEX/bamboo-plan-generator`; consult that repo when extending the file rather than guessing.
- **FSM annotations source** — `DEVEX/fsmannotations`. Source of the `@ModuleComponent` / `@ServiceComponent` / `@ProjectAppComponent` / `@WebAppComponent` / `@PublicComponent` / `@ScheduleTaskComponent` / `@UrlFactoryComponent` / `@GadgetComponent` annotations plus the `@Resource` / `@WebResource` meta-annotations. Pulled in at build time as `com.espirit.moddev.components:annotations` (version in `versions.properties`). Consult this repo when you need exact attribute names or defaults.
- **FirstSpirit API source** — `FS/firstspirit`. Module-side interfaces (`Service`, `ProjectApp`, `Module`, `WebApp`, `Configuration`, …) live under `firstspirit/fs/src/api/de/espirit/firstspirit/module/`; the `Executable` interface for `@PublicComponent` is at `firstspirit/fs/src/api/de/espirit/firstspirit/access/script/Executable.java`. Pulled in at build / fixture-compile time as `de.espirit.firstspirit:fs-isolated-runtime`.
